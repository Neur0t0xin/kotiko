/*
Импорт этого файла SCSS, чтобы объявить, что это зависимость в библиотеке.
Затем WebPack разделяет его в свой собственный файл CSS и включит в папку Dist.
*/
/*import './datepicker.scss'*/


var datepickers = [] // Получить переназначен в`remove()` ниже.
...
/*
 Функции обратного вызова по умолчанию (на выборе и т. Д.) Будут функции NOOP.
  Используя эту переменную, чтобы мы могли просто ссылаться на ту же функцию.
  Кроме того, это позволяет нам проверить, является ли обратный вызов функцией NOOP
  Делая `=== noop` везде, где нам нравится.
*/
function noop() { }
...
function createInstance(selectorOrElement, opts) {
    ...
    */
    ...
    var instance = {
        ...
        // Указывает, что это 2 -й экземпляр в паре Daterange.
...
/*
 *  Заполняет поля <Input> с читаемым значением
 * и хранит индивидуальные значения даты в качестве атрибутов.
 */
 ...
 * 2 сценария:
 *
 * Обновления `this.currentmonth` &` this.currentyear` на основе правых или левых стрел.
 * Создает `newdate` на основе обновленного месяца и года.
 * Вызывает `rendercalendar` с обновленной датой.
 *
 * Изменяет календарь на другой год
 * Из руководства пользователей ввода в наложении.
 * Вызывает `rendercalendar` с обновленной датой.
 */
...
/*
 * Программно устанавливает дату на экземпляре
 * и обновляет все связанные свойства.
 * Повторно заполнит календарь, если он покажет.
 */
function setDate(newDate, changeCalendar) {
    var date = stripTime(newDate) // Удалите время, создав свежий объект даты.
